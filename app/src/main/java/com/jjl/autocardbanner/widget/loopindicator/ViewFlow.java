...
import com.jjl.autocardbanner.R;
 ...
 * The default size of the buffer is banner3 elements on both sides of the currently
 * visible {@link View}, making up a total buffer size of banner3 * banner2 + banner1 = 7. The
...
public class ViewFlow extends AdapterView<Adapter> {
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_UP:
			...
			if (mTouchState == TOUCH_STATE_SCROLLING) {
				...
				// && mCurrentScreen == getChildCount() - banner1) {
				// snapToScreen(0);
				// }
				// else if (velocityX > SNAP_VELOCITY
				// && mCurrentScreen == 0) {
				// snapToScreen(getChildCount() - banner1);
				// }
				else {
					snapToDestination();
				}
	...
	private void postViewSwitched(int direction) {
		...
		if (direction > 0) { // to the right
			...
			// mCurrentAdapterIndex += mAdapter.getCount() - banner2;
			// mCurrentBufferIndex += mAdapter.getCount() - banner2;
			// }
